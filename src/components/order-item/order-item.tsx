...
const OrderItem = ({ path, item, linkState }: IOrderItemProps) => {
  ...
  const orderIngredientsMap: {
    [key: string]: { ingredient: IIngredient; count: number };
  } = {};
  ...
  const uniqueOrderIngredients = orderIngredients
    .filter((item, index) => {
      const currentIndex = orderIngredients.findIndex(
        (el) => el.ingredient._id === item.ingredient._id
      );
      return currentIndex === index;
    })
    .sort((a, b) => {
      if (a.ingredient.type === 'bun') {
        return -1;
      }
      if (b.ingredient.type === 'bun') {
        return 1;
      }
      return 0;
    });

  return uniqueOrderIngredients.length > 0 ? (
